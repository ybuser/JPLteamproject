...
public class Store {
	...
	public void draw(Graphics g) {
		...
		for(int i = 0; i < button.length; i++) {
			...
			if(buttonID[i] != Value.airAir) 
				g.drawImage(Screen.tileset_air[buttonID[i]], button[i].x + itemIn, button[i].y + itemIn, button[i].width - 2*itemIn, button[i].height - 2*itemIn, null);
		...
		g.setFont(new Font("Courler New", Font.BOLD, 14));
		g.setColor(new Color(255,255,255));
		...
		if(holdsItem) {
			g.drawImage(Screen.tileset_air[heldID], Screen.mse.x - ((button[0].width - itemIn*2)/2) +itemIn, Screen.mse.y - ((button[0].width - itemIn*2)/2) +itemIn, button[0].width - 2*itemIn, button[0].height - 2*itemIn, null);
		}
	 }
}